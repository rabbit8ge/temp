...
import com.example.database.DBUser.User;
...
public class DBHelper {
	 ...
	 */
	public long delete() {
		long id = db.delete(USER_TABLE_NAME, null, null);
		return id;
	}
		...
		if (db != null) {
			...
			if (count > 0) {
				...
				for (int i = 0; i < count; i++) {
					...
					mUserInfo.setUsername(cursor.getString(cursor.getColumnIndex(User.USERNAME)));
					mUserInfo.setPassword(cursor.getString(cursor.getColumnIndex(User.PASSWORD)));
					mList.add(mUserInfo);
					cursor.moveToNext();
				}
			}
	...
	private static class DBOpenHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("create table " + USER_TABLE_NAME + " (" + User._ID + " integer primary key," + User.USERNAME
					+ " text, " + User.PASSWORD + " text, " + User.ISSAVED + " INTEGER)");
		}
