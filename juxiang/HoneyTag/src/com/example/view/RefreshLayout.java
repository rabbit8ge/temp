...
/**
 * 继承自SwipeRefreshLayout,从�?�实现滑动到底部时上拉加载更多的功能.
 * 
 * @author mrsimple
 */
public class RefreshLayout extends SwipeRefreshLayout implements OnScrollListener {
	...
	private int mYDown;
	/**
	 * 抬起时的y坐标, 与mYDown�?起用于滑动到底部时判断是上拉还是下拉
	 */
	private int mLastY;
	/**
	 * 是否在加载中 ( 上拉加载更多 )
	...
	private void getListView() {
		...
		if (childs > 0) {
			...
			if (childView instanceof ListView) {
				...
				// 设置滚动监听器给ListView, 使得滚动的情况下也可以自动加�?
	...
	/**
	 * 是否可以加载更多, 条件是到了最底部, listview不在加载�?, 且为上拉操作.
	 * 
	 * @return
	 */
